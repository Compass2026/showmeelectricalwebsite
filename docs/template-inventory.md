...
| 12 | Contact, policies, optional careers | **Implemented; careers optional (Batch C); idempotency corrected (C1)** | `InquiryForm` + `/api/inquiry` (provider `Idempotency-Key` authoritative through `lib/email-provider.ts`, no local store; explicit changed/in-progress outcomes); `app/[doc]` legal route from the registry; careers only when `site.careers` is set | Show Me: careers host live; Harbor Lane: no careers (routes 404, no nav/sitemap/host rule) | — |
