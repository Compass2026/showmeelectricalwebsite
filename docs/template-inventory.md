...
| 12 | Contact, policies, optional careers | **Implemented; careers optional (Batch C); idempotency corrected (C1)** | `InquiryForm` + `/api/inquiry` (durable idempotency store + provider `Idempotency-Key`, explicit duplicate/changed/in-progress outcomes); `app/[doc]` legal route from the registry; careers only when `site.careers` is set | Show Me: careers host live; Harbor Lane: no careers (routes 404, no nav/sitemap/host rule) | — |
