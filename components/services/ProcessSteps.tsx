...
export default function ProcessSteps({ steps }: { steps: ProcessStep[] }) {
  return (
    <div className="relative mt-12">
      ...
      >
        <div className="mx-auto h-full w-2/3 bg-lime-500/60" />
      </div>

      <Reveal stagger as="ol" className="grid gap-10 md:grid-cols-3 md:gap-8">
        {steps.map((s) => (
          <li key={s.step} className="relative">
            <div className="flex h-14 w-14 items-center justify-center rounded-full border-2 border-lime-500 bg-navy-950 font-(family-name:--font-heading) text-sm font-bold text-lime-500 shadow-[0_0_24px_rgba(192,214,52,0.25)]">
              {s.step}
            </div>
